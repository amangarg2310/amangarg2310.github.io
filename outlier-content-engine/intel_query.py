...
Uses keyword-based retrieval + GPT reranking (no vector DB needed).
Flow: question → keyword search insights → GPT synthesizes answer with citations.
"""
...
def search_insights(domain_id: int, query: str, limit: int = 15, db_path=None) -> list[dict]:
    ...
    Simple but effective — searches title and content fields.
    """
    ...
    if not words:
        # Fallback: get most recent insights
        rows = conn.execute(
    ...
    # Build a scoring query using LIKE for each word
    conditions = []
    params = []
    for word in words:
    ...
    query_sql = f"""
        ...
        ORDER BY relevance DESC, i.id DESC
        LIMIT ?
    """

    # Params: scoring params + domain_id + filter params + limit
    scoring_params = []
    for word in words:
...
def query_domain(domain_id: int, question: str, db_path=None) -> dict:
    ...
    Returns:
        dict with: answer, sources_used, insights_found
    """
    db_path = db_path or config.DB_PATH
    ...
    conn = sqlite3.connect(str(db_path))
    conn.row_factory = sqlite3.Row
    domain = conn.execute("SELECT name FROM intel_domains WHERE id = ?", (domain_id,)).fetchone()
    if not domain:
        conn.close()
    ...
    synthesis_row = conn.execute(
        "SELECT content FROM intel_syntheses WHERE domain_id = ? ORDER BY version DESC LIMIT 1",
        (domain_id,),
    ).fetchone()
