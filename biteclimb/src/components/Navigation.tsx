...
export function Navigation() {
  return (
    <nav className="fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-md border-t border-neutral-100 px-2 py-1.5 z-50">
...
function NavItem({ to, icon, label }: { to: string; icon: ReactNode; label: string }) {
  ...
  return (
    ...
    >
      ...
      {isActive && (
        <div className="w-1 h-1 rounded-full bg-purple-600 mt-0.5 animate-scale-in" />
      )}
    </NavLink>
  )
}
