...
const DEV_URL = Platform.OS === 'android' ? 'http://10.0.2.2:3001/api' : 'http://localhost:3001/api';
const BASE_URL = __DEV__ ? DEV_URL : 'https://your-production-url.com/api';

async function getToken(): Promise<string | null> {
  try {
    return await SecureStore.getItemAsync('biteclimb_token');
  } catch {
    return null;
  }
}
...
export async function setToken(token: string): Promise<void> {
  await SecureStore.setItemAsync('biteclimb_token', token);
}

export async function removeToken(): Promise<void> {
  await SecureStore.deleteItemAsync('biteclimb_token');
}
