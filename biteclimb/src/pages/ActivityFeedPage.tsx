  ...
  return (
    <div className="max-w-md mx-auto px-4 py-6 page-enter">
      <header className="mb-5 animate-fade-in-up">
        ...
        <p className="text-neutral-500 text-sm">See what your product friends are up to</p>
      </header>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-12 text-neutral-500">
          ...
          <p className="text-sm mt-1">Follow other explorers to see their activity here</p>
        </div>
      ) : (
        <div className="space-y-2">
          {activities.map((activity, i) => (
            <Link
              key={activity.id}
              to={activity.type === 'follow' ? `/user/${activity.target_id}` : activity.type === 'tier_list' ? '/tier-builder' : `/product/${activity.target_id}`}
