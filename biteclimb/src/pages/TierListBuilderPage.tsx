...
import {
  SearchIcon, MapPinIcon, ChevronDownIcon, ShareIcon, DownloadIcon, XIcon, SaveIcon,
} from 'lucide-react'
import { TierBadge } from '../components/TierBadge'
import { api } from '../api/client'
import { TIER_OPTIONS } from '../data/types'
import type { TierType } from '../data/types'
import type { RestaurantData } from '../api/client'
...
export function TierListBuilderPage() {
  ...
  const [tierList, setTierList] = useState<Record<TierType, RestaurantData[]>>({
    S: [], A: [], B: [], C: [], D: [], F: [],
  })
  ...
  const saveMutation = useMutation({
    mutationFn: (data: { title: string; category: string; city: string; items: { restaurant_id: string; tier: string; sort_order: number }[] }) =>
  ...
  const rankedIds = useMemo(() => {
    ...
    for (const tier of TIER_OPTIONS) for (const r of tierList[tier]) ids.add(r.id)
    return ids
  }, [tierList])
  ...
  const unrankedRestaurants = restaurants.filter(
    r => !rankedIds.has(r.id) && r.name.toLowerCase().includes(searchTerm.toLowerCase())
  )

  const assignToTier = (restaurant: RestaurantData, tier: TierType) => {
    setTierList(prev => {
      const updated = { ...prev }
      for (const t of TIER_OPTIONS) updated[t] = updated[t].filter(r => r.id !== restaurant.id)
  ...
  const removeFromTier = (restaurantId: string, tier: TierType) => {
    setTierList(prev => ({ ...prev, [tier]: prev[tier].filter(r => r.id !== restaurantId) }))
  }

  const handleDragStart = (e: React.DragEvent, restaurant: RestaurantData, fromTier?: TierType) => {
  ...
  const handleDrop = (e: React.DragEvent, toTier: TierType) => {
    ...
    const restaurant = fromTier === 'unranked'
  ...
  const [assigningRestaurant, setAssigningRestaurant] = useState<RestaurantData | null>(null)
  ...
  const handleTouchEnd = () => {
    ...
    if (currentSwipeRestaurant) {
      if (swipeOffset > 100) {
        ...
        setTimeout(() => { assignToTier(currentSwipeRestaurant, 'S'); setSwipeExiting(null); setSwipeOffset(0) }, 250)
        return
  ...
  const handleSave = () => {
    const items: { restaurant_id: string; tier: string; sort_order: number }[] = []
    for (const tier of TIER_OPTIONS) {
      tierList[tier].forEach((r, i) => items.push({ restaurant_id: r.id, tier, sort_order: i }))
    }
    saveMutation.mutate({
      title: listTitle || `Best ${selectedCategory} in ${selectedCity}`,
  ...
  const handleShare = async () => {
    if (navigator.share) {
      await navigator.share({
        title: `My ${selectedCategory} Tier List - biteclimb`,
        text: `Check out my ${selectedCategory} tier list on biteclimb!`,
    ...
    <div className="min-h-screen bg-neutral-50 dark:bg-neutral-900 pb-20 page-enter">
      <div className="max-w-md mx-auto px-4 py-6 lg:max-w-6xl">
        <header className="mb-5">
          <h1 className="text-xl font-bold text-neutral-900 dark:text-neutral-100 mb-1">Rate the Best {selectedCategory} in {selectedCity}</h1>
          <div className="flex gap-3 mb-4">
            <button className="flex-1 flex items-center justify-between bg-white dark:bg-neutral-800 rounded-lg px-3 py-2 border border-neutral-200 dark:border-neutral-700 text-left text-sm active:scale-[0.98] transition-transform">
              <span className="font-medium dark:text-neutral-100">{selectedCategory}</span><ChevronDownIcon size={18} className="text-neutral-400" />
            </button>
            <button className="flex-1 flex items-center justify-between bg-white dark:bg-neutral-800 rounded-lg px-3 py-2 border border-neutral-200 dark:border-neutral-700 text-left text-sm active:scale-[0.98] transition-transform">
              <span className="font-medium dark:text-neutral-100">{selectedCity}</span><MapPinIcon size={18} className="text-neutral-400" />
            </button>
          </div>

          <div className="mb-3">
            <div className="flex items-center justify-between text-xs text-neutral-500 mb-1"><span>Progress</span><span className="font-medium">{totalRanked}/{restaurants.length} ranked</span></div>
            <div className="h-2 bg-neutral-200 dark:bg-neutral-700 rounded-full overflow-hidden">
              <div className="h-full bg-gradient-to-r from-purple-500 to-pink-500 rounded-full transition-all duration-700 ease-out" style={{ width: `${(totalRanked / Math.max(restaurants.length, 1)) * 100}%` }} />
            </div>
          </div>
            ...
            {currentSwipeRestaurant ? (
              <div className="mb-6">
                ...
                >
                  ...
                  <div className="h-52 w-full"><img src={currentSwipeRestaurant.image_url} alt={currentSwipeRestaurant.name} className="w-full h-full object-cover" /></div>
                  <div className="p-4">
                    <div className="flex items-center justify-between mb-1"><h3 className="font-bold text-lg dark:text-neutral-100">{currentSwipeRestaurant.name}</h3><TierBadge tier={currentSwipeRestaurant.community_tier as TierType} size="sm" showEmoji={false} /></div>
                    <p className="text-sm text-neutral-500 mb-2">{currentSwipeRestaurant.neighborhood} · {currentSwipeRestaurant.rating_count} community ratings</p>
                ...
                <div className="mt-4">
                  ...
                  <div className="flex justify-center gap-2">
                    {TIER_OPTIONS.map(tier => (
                      <button key={tier} onClick={() => { if (currentSwipeRestaurant) assignToTier(currentSwipeRestaurant, tier) }} className="transition-all duration-200 hover:scale-110 active:scale-90">
                ...
                {nonEmptyTiers.map(tier => (
                  <div key={tier} className="flex items-center gap-2">
                    ...
                    <div className="flex gap-1.5 flex-1 overflow-x-auto scrollbar-hide py-1">
                      {tierList[tier].map(r => (
                        <div key={r.id} className={`relative shrink-0 group ${lastAssigned?.id === r.id ? 'animate-bounce-in' : ''}`}>
                          <div className="w-10 h-10 rounded-lg overflow-hidden border border-neutral-200 dark:border-neutral-700"><img src={r.image_url} alt={r.name} className="w-full h-full object-cover" /></div>
                          <button onClick={() => removeFromTier(r.id, tier)} className="absolute -top-1 -right-1 w-4 h-4 bg-red-500 text-white rounded-full text-xs flex items-center justify-center opacity-0 group-hover:opacity-100 active:opacity-100 transition-opacity"><XIcon size={10} /></button>
                        </div>
                      ))}
                    </div>
                  </div>
        ...
        <div className={`${mode === 'grid' ? 'block' : 'hidden'} lg:block`}>
          <div className="flex flex-col lg:flex-row gap-6">
                  ...
                  <div className="flex-1 min-h-[72px] bg-white dark:bg-neutral-800 rounded-xl border-2 border-dashed border-neutral-200 dark:border-neutral-700 p-2 ml-3">
                    <div className="flex flex-wrap gap-2">
                      {tierList[tier].map(restaurant => (
                        <div key={restaurant.id} className={`relative group ${lastAssigned?.id === restaurant.id ? 'animate-bounce-in' : ''}`} draggable onDragStart={e => handleDragStart(e, restaurant, tier)}>
                          <div className="w-14 h-14 rounded-lg overflow-hidden border border-neutral-200 dark:border-neutral-700"><img src={restaurant.image_url} alt={restaurant.name} className="w-full h-full object-cover" loading="lazy" /></div>
                          <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity rounded-lg flex items-center justify-center"><span className="text-white text-[10px] font-medium px-1 text-center leading-tight">{restaurant.name}</span></div>
                          <button className="absolute -top-1 -right-1 w-4 h-4 bg-red-500 text-white rounded-full text-xs flex items-center justify-center opacity-0 group-hover:opacity-100 active:opacity-100 transition-opacity" onClick={() => removeFromTier(restaurant.id, tier)} aria-label={`Remove ${restaurant.name}`}><XIcon size={10} /></button>
                        </div>
                      ))}
            ...
            <div className="lg:w-80">
              <button className="lg:hidden w-full bg-purple-600 text-white font-medium py-3 rounded-xl mb-4 text-sm active:scale-[0.98] transition-transform" onClick={() => setShowRestaurants(!showRestaurants)}>{showRestaurants ? 'Hide Restaurants' : `Show Restaurants (${unrankedRestaurants.length})`}</button>
              <div className={`bg-white dark:bg-neutral-800 rounded-xl shadow-sm p-4 ${showRestaurants ? 'block' : 'hidden lg:block'}`}>
                <div className="relative mb-4">
                  <SearchIcon size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-neutral-400" />
                  <input type="text" placeholder="Search restaurants..." className="w-full pl-9 pr-4 py-2 rounded-lg border border-neutral-200 dark:border-neutral-700 bg-transparent dark:text-neutral-100 focus:outline-none focus:ring-2 focus:ring-purple-500 text-sm" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
                </div>
                <div className="space-y-3">
                  {unrankedRestaurants.map(restaurant => (
                    <div key={restaurant.id}>
                      <div className="flex items-center p-2 bg-neutral-50 dark:bg-neutral-700 rounded-lg cursor-move active:scale-[0.98] transition-transform" draggable onDragStart={e => handleDragStart(e, restaurant)} onClick={() => setAssigningRestaurant(assigningRestaurant?.id === restaurant.id ? null : restaurant)}>
                        <div className="w-14 h-14 rounded-lg overflow-hidden mr-3 shrink-0 border border-neutral-200 dark:border-neutral-700"><img src={restaurant.image_url} alt={restaurant.name} className="w-full h-full object-cover" loading="lazy" /></div>
                        <div className="flex-1 min-w-0">
                          <h3 className="font-medium text-sm text-neutral-900 dark:text-neutral-100">{restaurant.name}</h3>
                          <p className="text-xs text-neutral-500">{restaurant.neighborhood}</p>
                          <div className="flex items-center mt-1"><TierBadge tier={restaurant.community_tier as TierType} size="sm" showEmoji={false} /><span className="text-xs text-neutral-400 ml-2">{restaurant.rating_count}</span></div>
                        </div>
                      </div>
                      {assigningRestaurant?.id === restaurant.id && (
                        <div className="flex gap-1.5 mt-2 ml-2 pb-1 animate-fade-in">
                          {TIER_OPTIONS.map(tier => <button key={tier} onClick={() => { assignToTier(restaurant, tier); setAssigningRestaurant(null) }} className="transition-all duration-200 hover:scale-110 active:scale-90"><TierBadge tier={tier} size="sm" showEmoji={false} /></button>)}
                        </div>
                      )}
                    </div>
                  ))}
                  {unrankedRestaurants.length === 0 && <p className="text-sm text-neutral-500 text-center py-4">All restaurants ranked!</p>}
                </div>
              </div>
            </div>
          </div>
        ...
        {totalRanked > 0 && (
          <div className="mt-6 space-y-3">
            ...
            <div className="flex gap-2">
              <input
                type="text"
                placeholder={`Best ${selectedCategory} in ${selectedCity}`}
                value={listTitle}
                  ...
                  <div className="space-y-2">
                    {nonEmptyTiers.map(tier => (
                      <div key={tier} className="flex items-center gap-2">
                        ...
                        <div className="flex gap-1.5">{tierList[tier].map(r => <div key={r.id} className="w-8 h-8 rounded overflow-hidden"><img src={r.image_url} alt={r.name} className="w-full h-full object-cover" /></div>)}</div>
                        <span className="text-white/60 text-xs ml-auto truncate max-w-[120px]">{tierList[tier].map(r => r.name).join(', ')}</span>
                      </div>
                    ))}
                  </div>
                </div>
