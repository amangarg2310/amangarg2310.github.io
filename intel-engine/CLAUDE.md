...
- **Domain name collisions:** The same name can exist at different hierarchy levels (e.g. "AI Tools" at level-0 parent AND level-1 domain). `domain_page()` accepts `?level=N` query param to disambiguate. Default prefers level-1 (the actual domain, not the parent category). The knowledge tree bio-tree passes `?level=0` for category node links. Fallback: if exact level not found, uses deterministic ordering (prefer lowest level for level-0 requests, prefer level-1 otherwise).
- **Domain classification:** `DETECTION_PROMPT` in `domain_detector.py` classifies by PRIMARY TOPIC, not surface keywords. Tool-specific content → tool name domain. Conceptual/strategy content → descriptive topic domain. Only matches existing domains when the content is truly about the same subject — avoids forcing "How to build a 7-figure app" into "Mobile Apps" when it's really about entrepreneurship.
